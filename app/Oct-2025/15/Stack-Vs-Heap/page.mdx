...
> “How does *LIFO access (can’t pull plates from the middle)* logically imply that *everything on the stack must have known, fixed size*?”
...
## 🧮 Step 2: Why that forces *known, fixed sizes*
...
That’s what lets stack push/pop be *O(1)*.
...
Now the compiler doesn’t know *how many bytes* to reserve for `arr`.
...
But that “???” depends on `n`, which is only known *at runtime*.
...
But if you push a plate whose size you *don’t know until runtime*:
...
> “To *implement* LIFO efficiently with a single stack pointer,
...
> “Not being able to pull plates from between” describes *the freeing order constraint.*
