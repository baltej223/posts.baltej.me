...
const filenames = readdirSync("./storage").filter(
...
${mdxFile}
  ...
  const posts_json = {
    ...
    read: (number = "") => {
      return JSON.parse(
        readFileSync(inPostDir(`posts${number}.json`), "utf-8"),
      );
    },
    write: (number, obj) => {
      let jsn;
      try {
        ...
        throw new Error(
          "The object provided for writing, is not serilisable.",
          e,
        );
      }
