...
function LoadPosts() {
  ...
  return (
    <>
      <div className="flex flex-col gap-y-7">{content}</div>
    </>
  );
}
...
function GetNameFromUrl(url) {
  ...
  if (match) {
    ...
    throw new Error(
      "The URLs fetch from latest JSON file can't be parsed, Some Error when static site genrator. Frontend's clean.",
    );
  }
  return [name, date];
}
