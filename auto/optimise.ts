...
function inStorage(_path: Path): Path {
    let Storage_location : Path = "./storage";
    return path.join(path.resolve(__dirname,Storage_location), _path);
}

function inAppDirectory(_path:Path):Path{
  return path.normalize(path.join(path.resolve(__dirname, "../app"),path.normalize(_path)));
}
...
function isBuilt(time:string){
  let filename:any = posts[time];
  ...
  let absFileLocation:Path = inAppDirectory(fileLocation);
  // console.log("64",absFileLocation);
  // console.log("65", existsSync(absFileLocation));
  if (existsSync(absFileLocation)){
    console.log("\nLog: File :", filename, "is built at location:", absFileLocation, "\n");
    return true;
  }
  else{
    console.log("\nLog: File :", filename, "is **not** built at location:", absFileLocation, "\n");
    return false;
  }
}
...
postKeys.forEach((time_of_creation)=>{
  ...
  if (!(existsSync(inStorage(filename))+".mdx")){
    ...
    console.log(`The File: ${posts[filename]}'s entry is found in posts.json but it doesn't seems to be present in storage.\n`);
    console.log("Checking if it is built or not.\n");
    if (isBuilt(filename)){
      console.log("File is already built, safely ignoring posts.json\n");
    }
    else{
      console.log("Built file not found. Someone seems to manually edited the posts.json file, and forgot to delete the entry.\n");
    }
  }
  if (!isBuilt(filename)){
    throw new Error(`Error: File entry in posts.json from name "${posts[filename]}" is not built. Run "npm run build-posts;"`);
  }
