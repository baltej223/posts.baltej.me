export const metadata = {
  title: "Objects in C Lang | @baltej223",
  description: "With love and passion by Baltej Singh",
};

# Objects in C-Lang

- So first of all, can you write objects in C? Like C is not an object oriented language, but still you can get somewhat similar effect using structs.
...
int main(){
    struct MyStruct str;
    str.number=21;
    str.string = malloc(50);
    strcpy(str.string, "someRandomString");
}
```

- This object is not typical, unlike those in high-level languages like C++ or JS/TS. Typical objects contain a constructor, member functions or data members, and scopes like `public` or `private`.
...
- if we just focus on the basics and analyze the situation from the lens of functional programming, objects are group of some functions, providing a clear way and abstract way to use them.
- Although, it will be very wrong to say objects are only group of functions as there is more to them, They are usually functions + encapsulated state + identity + dispatch rules etc.
- Not deviating from the topic, So if we are able to somehow store function in C's `structs` **we can get objects in C Language.**
- For that, lets explore the concept of function pointers.

## Function Pointers

- As the name suggests, **pointer to a function is a function pointer.**
...
- If you have to declare a function pointer which stores the address of a function, which take in int and gives out int, it can be done like this:
